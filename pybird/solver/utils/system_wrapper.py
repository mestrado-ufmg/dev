...
from numpy import double, empty, int32, ndarray, zeros
...
def create(vertices: ndarray,
    ...
    outputStruct.matrix = convertArrayToPointer(empty((nf, nf)).astype(double))
    outputStruct.array = convertArrayToPointer(empty(nf).astype(double))
    outputStruct.velXMatrix = convertArrayToPointer(empty((nf, nf)).astype(double))
    outputStruct.velYMatrix = convertArrayToPointer(empty((nf, nf)).astype(double))
    outputStruct.velZMatrix = convertArrayToPointer(empty((nf, nf)).astype(double))
    outputStruct.velArray = convertArrayToPointer(empty((nf, 3)).astype(double))

    # Load the compiled library
    lib = npct.load_library('lib', './pybird/solver/utils/bin/')
    ...
    out = [
        ...
        convertPointerToArray(outputStruct.velXMatrix, [nf, nf]),
        convertPointerToArray(outputStruct.velYMatrix, [nf, nf]),
        convertPointerToArray(outputStruct.velZMatrix, [nf, nf]),

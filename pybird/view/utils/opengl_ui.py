...
class Geometry:
    ...
    edges: Tuple[Tuple]
    faces: Tuple[Tuple]
    const_color: Tuple[float] = (0.37, 0.45, 0.64)
...
class OpenGL_UI:
    ...
    def __init__(self, geos: List[Geometry] = [], wakes: List[Wake] = [], parts: List[np.ndarray] = [], colors: np.ndarray = None) -> None:
    ...
    def _get_color(self, face: int) -> Tuple:
        ...
        if self.colorsList is not None:
            return self.colorsList[face, :]
...
class OpenGL_UI_Vector_Field:
    
    def __init__(self, geo: Geometry = None, parts: List[np.ndarray] = [], vectors: List[np.ndarray] = None, positions: np.ndarray = None) -> None:
    ...
    def __geo_limits(self) -> None:
        max_x, max_y, max_z = -1, -1, -1
        for vertice in self.geo.vertices:
            if fabs(vertice[0]) > max_x: max_x = vertice[0]
            if fabs(vertice[1]) > max_y: max_y = vertice[1]
            if fabs(vertice[2]) > max_z: max_z = vertice[2]
        self.__max_x, self.__max_y, self.__max_z = max_x, max_y, max_z
    ...
    def __draw_surface_mesh(self) -> None:
        ...
        glBegin(GL_TRIANGLES)
        for i in range(len(self.geo.faces)):
            color = self._get_color(i)
            glColor3f(color[0], color[1], color[2])
            for vertex in self.geo.faces[i]:
                glVertex3fv(self.geo.vertices[vertex])
        glEnd()

        glBegin(GL_LINES)
        glColor3f(0, 0, 0)
        for edge in self.geo.edges:
            for vertex in edge:
                glVertex3fv(self.geo.vertices[vertex])
        
        for i in range(len(self.positions[:, 0])):
            for j in range(len(self.vectors)):
                glVertex3fv(self.positions[i, :])
                glVertex3fv(self.positions[i, :] + 0.05 * self.vectors[j][i, :])

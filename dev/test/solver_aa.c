...
void calculateObjectiveFunction2(int inter,
    ...
    /* Face integral equations */
    *momentum_x = (params.div_J_x - params.vel * params.div_M - params.tau_w_x) / (velocity * velocity);
    *momentum_y = (params.div_J_y - params.tau_w_y) / (velocity * velocity);
    *kinetic_energy = (params.div_E - params.vel * params.vel * params.div_M - params.density * (params.Q_x * params.grad_q2_x + params.Q_y * params.grad_q2_y) - 2 * params.D) / (velocity * velocity * velocity);
    *lateral_curvature = (params.div_K_o + (params.E_x * params.grad_phi_x + params.E_y * params.grad_phi_y) + 0.5 * params.density * (params.Q_x * params.grad_q2_y - params.Q_y * params.grad_q2_x) - params.density * (params.Q_o_x * params.grad_q2_x + params.Q_o_y * params.grad_q2_y) + params.D_x - 2 * params.D_o) / (velocity * velocity * velocity);
    *shear_stress_x = (params.div_K_tau_x - params.S_tau_x) / velocity;
    *shear_stress_y = (params.div_K_tau_y - params.S_tau_y) / velocity;
    ...
    double f2 = 1.0; // + 0.5 * sin((inter - 20) * 2 * PI / 100);
...
void solveBoundaryLayer(int nf,
    ...
    double max_values[6] = {0.01, 0.001, 0.001, 0.001, 0.000001, 0.000001};
    double max_step;
    ...
    double div_M, div_Jx, div_Jy, div_E, div_Ko, div_Ktaux, div_Ktauy;
    double div_M_1, div_Jx_1, div_Jy_1, div_E_1, div_Ko_1, div_Ktaux_1, div_Ktauy_1;
    double div_M_2, div_Jx_2, div_Jy_2, div_E_2, div_Ko_2, div_Ktaux_2, div_Ktauy_2;
    double div_M_3, div_Jx_3, div_Jy_3, div_E_3, div_Ko_3, div_Ktaux_3, div_Ktauy_3;
    double div_M_aux, div_Jx_aux, div_Jy_aux, div_E_aux, div_Ko_aux, div_Ktaux_aux, div_Ktauy_aux;
    int face_index;
    double div_factor = 0.0;
    struct EquationsParameters *div_params = (struct EquationsParameters *)malloc(30 * sizeof(struct EquationsParameters));
    int *div_faces = (int *)malloc(30 * sizeof(int));
    int div_count;
    ...
    for (i = 0; i < int_max; i++) {
        ...
        /* Calculate Divergents and gradients */
        for (j = 0; j < nf; j++) {
        ...
        for (j = 0; j < nf; j++) {
            ...
            /* Divergent diffusion */
            face_index = j;
            div_M_aux = params[face_index].div_M; div_Jx_aux = params[face_index].div_J_x; div_Jy_aux = params[face_index].div_J_y; div_E_aux = params[face_index].div_E; div_Ko_aux = params[face_index].div_K_o; div_Ktaux_aux = params[face_index].div_K_tau_x; div_Ktauy_aux = params[face_index].div_K_tau_y;
            div_M_1 = 0.0; div_Jx_1 = 0.0; div_Jy_1 = 0.0; div_E_1 = 0.0; div_Ko_1 = 0.0; div_Ktaux_1 = 0.0; div_Ktauy_1 = 0.0;
            div_M_2 = 0.0; div_Jx_2 = 0.0; div_Jy_2 = 0.0; div_E_2 = 0.0; div_Ko_2 = 0.0; div_Ktaux_2 = 0.0; div_Ktauy_2 = 0.0;
            div_M_3 = 0.0; div_Jx_3 = 0.0; div_Jy_3 = 0.0; div_E_3 = 0.0; div_Ko_3 = 0.0; div_Ktaux_3 = 0.0; div_Ktauy_3 = 0.0;
            
            face_index = faces[3 * j];
            for (k = 0; k < vertices_connection[face_index].n; k++) {
                div_M_1 = div_M_1 + params[vertices_connection[face_index].faces[k]].div_M * vertices_connection[face_index].coeffs[k];
                div_Jx_1 = div_Jx_1 + params[vertices_connection[face_index].faces[k]].div_J_x * vertices_connection[face_index].coeffs[k];
                div_Jy_1 = div_Jy_1 + params[vertices_connection[face_index].faces[k]].div_J_y * vertices_connection[face_index].coeffs[k];
                div_E_1 = div_E_1 + params[vertices_connection[face_index].faces[k]].div_E * vertices_connection[face_index].coeffs[k];
                div_Ko_1 = div_Ko_1 + params[vertices_connection[face_index].faces[k]].div_K_o * vertices_connection[face_index].coeffs[k];
                div_Ktaux_1 = div_Ktaux_1 + params[vertices_connection[face_index].faces[k]].div_K_tau_x * vertices_connection[face_index].coeffs[k];
                div_Ktauy_1 = div_Ktauy_1 + params[vertices_connection[face_index].faces[k]].div_K_tau_y * vertices_connection[face_index].coeffs[k];
            }

            face_index = faces[3 * j + 1];
            for (k = 0; k < vertices_connection[face_index].n; k++) {
                div_M_2 = div_M_2 + params[vertices_connection[face_index].faces[k]].div_M * vertices_connection[face_index].coeffs[k];
                div_Jx_2 = div_Jx_2 + params[vertices_connection[face_index].faces[k]].div_J_x * vertices_connection[face_index].coeffs[k];
                div_Jy_2 = div_Jy_2 + params[vertices_connection[face_index].faces[k]].div_J_y * vertices_connection[face_index].coeffs[k];
                div_E_2 = div_E_2 + params[vertices_connection[face_index].faces[k]].div_E * vertices_connection[face_index].coeffs[k];
                div_Ko_2 = div_Ko_2 + params[vertices_connection[face_index].faces[k]].div_K_o * vertices_connection[face_index].coeffs[k];
                div_Ktaux_2 = div_Ktaux_2 + params[vertices_connection[face_index].faces[k]].div_K_tau_x * vertices_connection[face_index].coeffs[k];
                div_Ktauy_2 = div_Ktauy_2 + params[vertices_connection[face_index].faces[k]].div_K_tau_y * vertices_connection[face_index].coeffs[k];
            }

            face_index = faces[3 * j + 2];
            for (k = 0; k < vertices_connection[face_index].n; k++) {
                div_M_3 = div_M_3 + params[vertices_connection[face_index].faces[k]].div_M * vertices_connection[face_index].coeffs[k];
                div_Jx_3 = div_Jx_3 + params[vertices_connection[face_index].faces[k]].div_J_x * vertices_connection[face_index].coeffs[k];
                div_Jy_3 = div_Jy_3 + params[vertices_connection[face_index].faces[k]].div_J_y * vertices_connection[face_index].coeffs[k];
                div_E_3 = div_E_3 + params[vertices_connection[face_index].faces[k]].div_E * vertices_connection[face_index].coeffs[k];
                div_Ko_3 = div_Ko_3 + params[vertices_connection[face_index].faces[k]].div_K_o * vertices_connection[face_index].coeffs[k];
                div_Ktaux_3 = div_Ktaux_3 + params[vertices_connection[face_index].faces[k]].div_K_tau_x * vertices_connection[face_index].coeffs[k];
                div_Ktauy_3 = div_Ktauy_3 + params[vertices_connection[face_index].faces[k]].div_K_tau_y * vertices_connection[face_index].coeffs[k];
            }

            face_index = j;
            params[face_index].div_M = (params[face_index].div_M + div_factor * (div_M_1 + div_M_2 + div_M_3)) / (div_factor + 1);
            params[face_index].div_J_x = (params[face_index].div_J_x + div_factor * (div_Jx_1 + div_Jx_2 + div_Jx_3)) / (div_factor + 1);
            params[face_index].div_J_y = (params[face_index].div_J_y + div_factor * (div_Jy_1 + div_Jy_2 + div_Jy_3)) / (div_factor + 1);
            params[face_index].div_E = (params[face_index].div_E + div_factor * (div_E_1 + div_E_2 + div_E_3)) / (div_factor + 1);
            params[face_index].div_K_o = (params[face_index].div_K_o + div_factor * (div_Ko_1 + div_Ko_2 + div_Ko_3)) / (div_factor + 1);
            params[face_index].div_K_tau_x = (params[face_index].div_K_tau_x + div_factor * (div_Ktaux_1 + div_Ktaux_2 + div_Ktaux_3)) / (div_factor + 1);
            params[face_index].div_K_tau_y = (params[face_index].div_K_tau_y + div_factor * (div_Ktauy_1 + div_Ktauy_2 + div_Ktauy_3)) / (div_factor + 1);
            ...
            params[face_index].div_M = div_M_aux;
            params[face_index].div_J_x = div_Jx_aux;
            params[face_index].div_J_y = div_Jy_aux;
            params[face_index].div_E = div_E_aux;
            params[face_index].div_K_o = div_Ko_aux;
            params[face_index].div_K_tau_x = div_Ktaux_aux;
            params[face_index].div_K_tau_y = div_Ktauy_aux;
            ...
            for (l = 0; l < faces_connection[j].n; l++) {

                div_count = 0;
                for (n = 0; n < faces_connection[j].n; n++) {
                    for (m = 0; m < faces_connection[faces_connection[j].faces[l]].n; m++) {
                        if (faces_connection[j].faces[n] == faces_connection[faces_connection[j].faces[l]].faces[m]) {
                            div_faces[div_count] = faces_connection[j].faces[n];
                            div_params[div_count] = params[faces_connection[j].faces[n]];
                ...
                /* Save current face params */
                params_aux = params[faces_connection[j].faces[l]];
                ...
                /* Add delta eps params */
                params[faces_connection[j].faces[l]] = params_delta_eps[faces_connection[j].faces[l]];

                /* Calculate divergent and gradient */
                // for (m = 0; m < div_count; m++) {
                //     calculateDivergents(div_faces[m], faces, vertices_connection, params, facesArea[div_faces[m]], p1, p2, p3);
                //     calculateGradients(div_faces[m], faces, vertices_connection, params, e1, e2, e3, p1, p2, p3, velNorm, velx, vely, velz, transpiration);
                // }
                calculateDivergents(j, faces, vertices_connection, params, facesArea[j], p1, p2, p3);
                calculateGradients(j, faces, vertices_connection, params, e1, e2, e3, p1, p2, p3, velNorm, velx, vely, velz, transpiration);
                ...
                /* Obj. func. with delta eps */
                calculateObjectiveFunction2(i, params[j], &obj_func[7 + 6 * l], &max_momentum_x_aux, &max_momentum_y_aux, &max_kinetic_energy_aux, &max_lateral_curvature_aux, &max_shear_stress_x_aux, &max_shear_stress_y_aux, freestreamNorm);
                ...
                for (m = 0; m < div_count; m++) params[div_faces[m]] = div_params[div_count];
                ...
                /* Add delta eps params */
                params[faces_connection[j].faces[l]] = params_A_eps[faces_connection[j].faces[l]];
                ...
                /* Obj. func. with delta eps */
                calculateObjectiveFunction2(i, params[j], &obj_func[7 + 6 * l + 1], &max_momentum_x_aux, &max_momentum_y_aux, &max_kinetic_energy_aux, &max_lateral_curvature_aux, &max_shear_stress_x_aux, &max_shear_stress_y_aux, freestreamNorm);
                ...
                /* Add delta eps params */
                params[faces_connection[j].faces[l]] = params_B_eps[faces_connection[j].faces[l]];
                ...
                /* Obj. func. with delta eps */
                calculateObjectiveFunction2(i, params[j], &obj_func[7 + 6 * l + 2], &max_momentum_x_aux, &max_momentum_y_aux, &max_kinetic_energy_aux, &max_lateral_curvature_aux, &max_shear_stress_x_aux, &max_shear_stress_y_aux, freestreamNorm);
                ...
                /* Add delta eps params */
                params[faces_connection[j].faces[l]] = params_Psi_eps[faces_connection[j].faces[l]];
                ...
                /* Obj. func. with delta eps */
                calculateObjectiveFunction2(i, params[j], &obj_func[7 + 6 * l + 3], &max_momentum_x_aux, &max_momentum_y_aux, &max_kinetic_energy_aux, &max_lateral_curvature_aux, &max_shear_stress_x_aux, &max_shear_stress_y_aux, freestreamNorm);
                ...
                /* Add delta eps params */
                params[faces_connection[j].faces[l]] = params_Ctau1_eps[faces_connection[j].faces[l]];
                ...
                /* Obj. func. with delta eps */
                calculateObjectiveFunction2(i, params[j], &obj_func[7 + 6 * l + 4], &max_momentum_x_aux, &max_momentum_y_aux, &max_kinetic_energy_aux, &max_lateral_curvature_aux, &max_shear_stress_x_aux, &max_shear_stress_y_aux, freestreamNorm);
                ...
                /* Add delta eps params */
                params[faces_connection[j].faces[l]] = params_Ctau2_eps[faces_connection[j].faces[l]];
                ...
                /* Obj. func. with delta eps */
                calculateObjectiveFunction2(i, params[j], &obj_func[7 + 6 * l + 5], &max_momentum_x_aux, &max_momentum_y_aux, &max_kinetic_energy_aux, &max_lateral_curvature_aux, &max_shear_stress_x_aux, &max_shear_stress_y_aux, freestreamNorm);
                ...
                params[faces_connection[j].faces[l]] = params_aux;
        ...
        //         step_ref = step_ref * 1.001;
        //     } else {
        //         step_ref = step_ref / 1.5;
        //     }
        ...
        if (i >= 7) {
            ...
            //     (absValue(division(max_error_list[i - 4], max_error_list[i - 5])) > 1)) {

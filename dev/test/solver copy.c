...
void calculateObjectiveFunction2(int inter,
    ...
    *shear_stress_y = params.div_K_tau_y - params.S_tau_y;
    ...
    double f2 = 0.0; // + 0.5 * sin((inter - 20) * 2 * PI / 100);
    double f3 = 0.0; // + 0.2 * sin((inter + 50) * 2 * PI / 100);
    double f4 = 0.0; // + 1 * sin((inter + 75) * 2 * PI / 100);
...
void solveBoundaryLayer(int nf,
    ...
    for (i = 0; i < int_max; i++) {
        ...
        if (i > 0) {
            ...
            for(j = 0; j < nf; j++) {
                norm_delta_list[j] = (5 * norm_delta_list[j] + norm_delta_list_aux[faces[3 * j]] + norm_delta_list_aux[faces[3 * j + 1]] + norm_delta_list_aux[faces[3 * j + 2]]) / 8;
                norm_A_list[j] = (5 * norm_A_list[j] + norm_A_list_aux[faces[3 * j]] + norm_A_list_aux[faces[3 * j + 1]] + norm_A_list_aux[faces[3 * j + 2]]) / 8;
                norm_B_list[j] = (5 * norm_B_list[j] + norm_B_list_aux[faces[3 * j]] + norm_B_list_aux[faces[3 * j + 1]] + norm_B_list_aux[faces[3 * j + 2]]) / 8;
                norm_Psi_list[j] = (5 * norm_Psi_list[j] + norm_Psi_list_aux[faces[3 * j]] + norm_Psi_list_aux[faces[3 * j + 1]] + norm_Psi_list_aux[faces[3 * j + 2]]) / 8;
                norm_Ctau1_list[j] = (5 * norm_Ctau1_list[j] + norm_Ctau1_list_aux[faces[3 * j]] + norm_Ctau1_list_aux[faces[3 * j + 1]] + norm_Ctau1_list_aux[faces[3 * j + 2]]) / 8;
                norm_Ctau2_list[j] = (5 * norm_Ctau2_list[j] + norm_Ctau2_list_aux[faces[3 * j]] + norm_Ctau2_list_aux[faces[3 * j + 1]] + norm_Ctau2_list_aux[faces[3 * j + 2]]) / 8;
            }

        }

        /* Print error */
        printf("           %d        %.4e      %.4e       %.4e      %.4e       %.4e      %.4e      %.4e\n", i + 1, max_error / max_obj_func, max_momentum_x, max_momentum_y, max_kinetic_energy, max_lateral_curvature, max_shear_stress_x, max_shear_stress_y);

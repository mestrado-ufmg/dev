...
void solveBoundaryLayer(int nf,
    ...
    max_inc[0] = 0.1; max_inc[1] = 0.1; max_inc[2] = 0.1; max_inc[3] = 0.1; max_inc[4] = 0.1; max_inc[5] = 0.1;
    ...
    for (i = 1; i <= int_max; i++) {
        ...
        for (j = 0; j < nf; j++) {
            ...
            for (k = 0; k < 6 * n_local_faces; k++) ga_grad[k] = (max_error) * max_step * grad_local[k];
            ga_max_inter_ga = 10; // (int) ceil(log(0.01) / log((sqrt(5) - 1) / 2));
            // ga_max_inter_ga = (int) ceil(log(1e-4 / normVec(6 * n_local_faces, ga_grad)) / log((sqrt(5) - 1) / 2));
        ...
        for (j = 0; j < nf; j++) {
            ...
            if (x[6 * j + 4] < 1e-14) x[6 * j + 4] = 1e-14;
            if (x[6 * j + 5] < 1e-14) x[6 * j + 5] = 1e-14;
        }

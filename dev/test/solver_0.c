...
void calculateObjectiveFunction2(int inter,
    ...
    /* Face integral equations */
    *momentum_x = (params.div_J_x - params.vel * params.div_M - params.tau_w_x) / ((params.vel + 1e-3) * (params.vel + 1e-3) * params.density);
    *momentum_y = (params.div_J_y - params.tau_w_y) / ((params.vel + 1e-3) * (params.vel + 1e-3) * params.density);
    *kinetic_energy = (params.div_E - params.vel * params.vel * params.div_M - params.density * (params.Q_x * params.grad_q2_x + params.Q_y * params.grad_q2_y) - 2 * params.D) / ((params.vel + 1e-3) * (params.vel + 1e-3) * (params.vel + 1e-3) * params.density);
    *lateral_curvature = (params.div_K_o + (params.E_x * params.grad_phi_x + params.E_y * params.grad_phi_y) + 0.5 * params.density * (params.Q_x * params.grad_q2_y - params.Q_y * params.grad_q2_x) - params.density * (params.Q_o_x * params.grad_q2_x + params.Q_o_y * params.grad_q2_y) + params.D_x - 2 * params.D_o) / ((params.vel + 1e-3) * (params.vel + 1e-3) * (params.vel + 1e-3) * params.density);
    *shear_stress_x = (params.div_K_tau_x - params.S_tau_x) / (params.vel + 1e-3);
    *shear_stress_y = (params.div_K_tau_y - params.S_tau_y) / (params.vel + 1e-3);
    ...
    // *obj = pow(*momentum_x, 2) + pow(*momentum_y, 2) + pow(*kinetic_energy * 1e-2 / (velocity * velocity), 2) + pow(*lateral_curvature * 1e-3 / (velocity), 2) + pow(*shear_stress_x * 1e-3, 2) + pow(*shear_stress_y * 1e-3, 2);
    double f1 = 1.0; // + 0.05 * sin((inter + 149) * 2 * PI / 100);
    double f2 = 2.0; // + 0.5 * sin((inter - 20) * 2 * PI / 100);
    double f3 = 1.0; // + 0.2 * sin((inter + 50) * 2 * PI / 100);
    double f4 = 1.0; // + 1 * sin((inter + 75) * 2 * PI / 100);
    *obj = f1 * pow(*momentum_x, 2) + f1 * pow(*momentum_y, 2) + f2 * pow(*kinetic_energy, 2) + f3 * pow(*lateral_curvature, 2) + f4 * pow(*shear_stress_x, 2) + f4 * pow(*shear_stress_y, 2);

}

void avgValues(int face,
               int *faces,
               struct FacesConnection *faces_connection,
               struct VerticeConnection *vertices_connection,
               struct EquationsParameters *params,
               double *values) {
    ...
    // Save values
    values[0] = params[face].M_x; values[1] = params[face].M_y;
    values[2] = params[face].E_x; values[3] = params[face].E_y;
    values[4] = params[face].K_o_x; values[5] = params[face].K_o_y;
    values[6] = params[face].K_tau_xx; values[7] = params[face].K_tau_xy;
    values[8] = params[face].K_tau_yx; values[9] = params[face].K_tau_yy;
    values[10] = params[face].J_xx; values[11] = params[face].J_xy;
    values[12] = params[face].J_yx; values[13] = params[face].J_yy;

    for (i = 0; i < faces_connection[face].n; i++) {
        values[14 + 14 * i] = params[face].M_x; values[15 + 14 * i] = params[face].M_y;
        values[16 + 14 * i] = params[face].E_x; values[17 + 14 * i] = params[face].E_y;
        values[18 + 14 * i] = params[face].K_o_x; values[19 + 14 * i] = params[face].K_o_y;
        values[20 + 14 * i] = params[face].K_tau_xx; values[21 + 14 * i] = params[face].K_tau_xy;
        values[22 + 14 * i] = params[face].K_tau_yx; values[23 + 14 * i] = params[face].K_tau_yy;
        values[24 + 14 * i] = params[face].J_xx; values[25 + 14 * i] = params[face].J_xy;
        values[26 + 14 * i] = params[face].J_yx; values[27 + 14 * i] = params[face].J_yy;
    }

    int vertice_index;
    double *M_x = (double*)calloc(3, sizeof(double));
    double *M_y = (double*)calloc(3, sizeof(double));
    double *E_x = (double*)calloc(3, sizeof(double));
    double *E_y = (double*)calloc(3, sizeof(double));
    double *ko_x = (double*)calloc(3, sizeof(double));
    double *ko_y = (double*)calloc(3, sizeof(double));
    double *Ktau_xx = (double*)calloc(3, sizeof(double));
    double *Ktau_xy = (double*)calloc(3, sizeof(double));
    double *Ktau_yx = (double*)calloc(3, sizeof(double));
    double *Ktau_yy = (double*)calloc(3, sizeof(double));
    double *J_xx = (double*)calloc(3, sizeof(double));
    double *J_xy = (double*)calloc(3, sizeof(double));
    double *J_yx = (double*)calloc(3, sizeof(double));
    double *J_yy = (double*)calloc(3, sizeof(double));

    double factor = 2000.0;

    // Diffusion
    vertice_index = faces[3 * face];
    for (i = 0; i < vertices_connection[vertice_index].n; i++) {
        M_x[0] = M_x[0] + params[vertices_connection[vertice_index].faces[i]].M_x * vertices_connection[vertice_index].coeffs[i];
        M_y[0] = M_y[0] + params[vertices_connection[vertice_index].faces[i]].M_y * vertices_connection[vertice_index].coeffs[i];
        E_x[0] = E_x[0] + params[vertices_connection[vertice_index].faces[i]].E_x * vertices_connection[vertice_index].coeffs[i];
        E_y[0] = E_y[0] + params[vertices_connection[vertice_index].faces[i]].E_y * vertices_connection[vertice_index].coeffs[i];
        ko_x[0] = ko_x[0] + params[vertices_connection[vertice_index].faces[i]].K_o_x * vertices_connection[vertice_index].coeffs[i];
        ko_y[0] = ko_y[0] + params[vertices_connection[vertice_index].faces[i]].K_o_y * vertices_connection[vertice_index].coeffs[i];
        Ktau_xx[0] = Ktau_xx[0] + params[vertices_connection[vertice_index].faces[i]].K_tau_xx * vertices_connection[vertice_index].coeffs[i];
        Ktau_xy[0] = Ktau_xy[0] + params[vertices_connection[vertice_index].faces[i]].K_tau_xy * vertices_connection[vertice_index].coeffs[i];
        Ktau_yx[0] = Ktau_yx[0] + params[vertices_connection[vertice_index].faces[i]].K_tau_yx * vertices_connection[vertice_index].coeffs[i];
        Ktau_yy[0] = Ktau_yy[0] + params[vertices_connection[vertice_index].faces[i]].K_tau_yy * vertices_connection[vertice_index].coeffs[i];
        J_xx[0] = J_xx[0] + params[vertices_connection[vertice_index].faces[i]].J_xx * vertices_connection[vertice_index].coeffs[i];
        J_xy[0] = J_xy[0] + params[vertices_connection[vertice_index].faces[i]].J_xy * vertices_connection[vertice_index].coeffs[i];
        J_yx[0] = J_yx[0] + params[vertices_connection[vertice_index].faces[i]].J_yx * vertices_connection[vertice_index].coeffs[i];
        J_yy[0] = J_yy[0] + params[vertices_connection[vertice_index].faces[i]].J_yy * vertices_connection[vertice_index].coeffs[i];
    }

    vertice_index = faces[3 * face + 1];
    for (i = 0; i < vertices_connection[vertice_index].n; i++) {
        M_x[1] = M_x[1] + params[vertices_connection[vertice_index].faces[i]].M_x * vertices_connection[vertice_index].coeffs[i];
        M_y[1] = M_y[1] + params[vertices_connection[vertice_index].faces[i]].M_y * vertices_connection[vertice_index].coeffs[i];
        E_x[1] = E_x[1] + params[vertices_connection[vertice_index].faces[i]].E_x * vertices_connection[vertice_index].coeffs[i];
        E_y[1] = E_y[1] + params[vertices_connection[vertice_index].faces[i]].E_y * vertices_connection[vertice_index].coeffs[i];
        ko_x[1] = ko_x[1] + params[vertices_connection[vertice_index].faces[i]].K_o_x * vertices_connection[vertice_index].coeffs[i];
        ko_y[1] = ko_y[1] + params[vertices_connection[vertice_index].faces[i]].K_o_y * vertices_connection[vertice_index].coeffs[i];
        Ktau_xx[1] = Ktau_xx[1] + params[vertices_connection[vertice_index].faces[i]].K_tau_xx * vertices_connection[vertice_index].coeffs[i];
        Ktau_xy[1] = Ktau_xy[1] + params[vertices_connection[vertice_index].faces[i]].K_tau_xy * vertices_connection[vertice_index].coeffs[i];
        Ktau_yx[1] = Ktau_yx[1] + params[vertices_connection[vertice_index].faces[i]].K_tau_yx * vertices_connection[vertice_index].coeffs[i];
        Ktau_yy[1] = Ktau_yy[1] + params[vertices_connection[vertice_index].faces[i]].K_tau_yy * vertices_connection[vertice_index].coeffs[i];
        J_xx[1] = J_xx[1] + params[vertices_connection[vertice_index].faces[i]].J_xx * vertices_connection[vertice_index].coeffs[i];
        J_xy[1] = J_xy[1] + params[vertices_connection[vertice_index].faces[i]].J_xy * vertices_connection[vertice_index].coeffs[i];
        J_yx[1] = J_yx[1] + params[vertices_connection[vertice_index].faces[i]].J_yx * vertices_connection[vertice_index].coeffs[i];
        J_yy[1] = J_yy[1] + params[vertices_connection[vertice_index].faces[i]].J_yy * vertices_connection[vertice_index].coeffs[i];
    }

    vertice_index = faces[3 * face + 2];
    for (i = 0; i < vertices_connection[vertice_index].n; i++) {
        M_x[2] = M_x[2] + params[vertices_connection[vertice_index].faces[i]].M_x * vertices_connection[vertice_index].coeffs[i];
        M_y[2] = M_y[2] + params[vertices_connection[vertice_index].faces[i]].M_y * vertices_connection[vertice_index].coeffs[i];
        E_x[2] = E_x[2] + params[vertices_connection[vertice_index].faces[i]].E_x * vertices_connection[vertice_index].coeffs[i];
        E_y[2] = E_y[2] + params[vertices_connection[vertice_index].faces[i]].E_y * vertices_connection[vertice_index].coeffs[i];
        ko_x[2] = ko_x[2] + params[vertices_connection[vertice_index].faces[i]].K_o_x * vertices_connection[vertice_index].coeffs[i];
        ko_y[2] = ko_y[2] + params[vertices_connection[vertice_index].faces[i]].K_o_y * vertices_connection[vertice_index].coeffs[i];
        Ktau_xx[2] = Ktau_xx[2] + params[vertices_connection[vertice_index].faces[i]].K_tau_xx * vertices_connection[vertice_index].coeffs[i];
        Ktau_xy[2] = Ktau_xy[2] + params[vertices_connection[vertice_index].faces[i]].K_tau_xy * vertices_connection[vertice_index].coeffs[i];
        Ktau_yx[2] = Ktau_yx[2] + params[vertices_connection[vertice_index].faces[i]].K_tau_yx * vertices_connection[vertice_index].coeffs[i];
        Ktau_yy[2] = Ktau_yy[2] + params[vertices_connection[vertice_index].faces[i]].K_tau_yy * vertices_connection[vertice_index].coeffs[i];
        J_xx[2] = J_xx[2] + params[vertices_connection[vertice_index].faces[i]].J_xx * vertices_connection[vertice_index].coeffs[i];
        J_xy[2] = J_xy[2] + params[vertices_connection[vertice_index].faces[i]].J_xy * vertices_connection[vertice_index].coeffs[i];
        J_yx[2] = J_yx[2] + params[vertices_connection[vertice_index].faces[i]].J_yx * vertices_connection[vertice_index].coeffs[i];
        J_yy[2] = J_yy[2] + params[vertices_connection[vertice_index].faces[i]].J_yy * vertices_connection[vertice_index].coeffs[i];
    }

    params[face].M_x = (factor * params[face].M_x + M_x[0] + M_x[1] + M_x[2]) / (factor + 3);
    params[face].M_y = (factor * params[face].M_y + M_y[0] + M_y[1] + M_y[2]) / (factor + 3);
    params[face].E_x = (factor * params[face].E_x + E_x[0] + E_x[1] + E_x[2]) / (factor + 3);
    params[face].E_y = (factor * params[face].E_y + E_y[0] + E_y[1] + E_y[2]) / (factor + 3);
    params[face].K_o_x = (factor * params[face].K_o_x + ko_x[0] + ko_x[1] + ko_x[2]) / (factor + 3);
    params[face].K_o_y = (factor * params[face].K_o_y + ko_y[0] + ko_y[1] + ko_y[2]) / (factor + 3);
    params[face].K_tau_xx = (factor * params[face].K_tau_xx + Ktau_xx[0] + Ktau_xx[1] + Ktau_xx[2]) / (factor + 3);
    params[face].K_tau_xy = (factor * params[face].K_tau_xy + Ktau_xy[0] + Ktau_xy[1] + Ktau_xy[2]) / (factor + 3);
    params[face].K_tau_yx = (factor * params[face].K_tau_yx + Ktau_yx[0] + Ktau_yx[1] + Ktau_yx[2]) / (factor + 3);
    params[face].K_tau_yy = (factor * params[face].K_tau_yy + Ktau_yy[0] + Ktau_yy[1] + Ktau_yy[2]) / (factor + 3);
        
    for (j = 0; j < faces_connection[face].n; j++) {

        M_x[0] = 0.0; M_x[1] = 0.0; M_x[2] = 0.0;
        M_y[0] = 0.0; M_y[1] = 0.0; M_y[2] = 0.0;
        E_x[0] = 0.0; E_x[1] = 0.0; E_x[2] = 0.0;
        E_y[0] = 0.0; E_y[1] = 0.0; E_y[2] = 0.0;
        ko_x[0] = 0.0; ko_x[1] = 0.0; ko_x[2] = 0.0;
        ko_y[0] = 0.0; ko_y[1] = 0.0; ko_y[2] = 0.0;
        Ktau_xx[0] = 0.0; Ktau_xx[1] = 0.0; Ktau_xx[2] = 0.0;
        Ktau_xy[0] = 0.0; Ktau_xy[1] = 0.0; Ktau_xy[2] = 0.0;
        Ktau_yx[0] = 0.0; Ktau_yx[1] = 0.0; Ktau_yx[2] = 0.0;
        Ktau_yy[0] = 0.0; Ktau_yy[1] = 0.0; Ktau_yy[2] = 0.0;
        J_xx[0] = 0.0; J_xx[1] = 0.0; J_xx[2] = 0.0;
        J_xy[0] = 0.0; J_xy[1] = 0.0; J_xy[2] = 0.0;
        J_yx[0] = 0.0; J_yx[1] = 0.0; J_yx[2] = 0.0;
        J_yy[0] = 0.0; J_yy[1] = 0.0; J_yy[2] = 0.0;
        
        vertice_index = faces[3 * faces_connection[face].faces[i]];
        for (i = 0; i < vertices_connection[vertice_index].n; i++) {
            M_x[0] = M_x[0] + params[vertices_connection[vertice_index].faces[i]].M_x * vertices_connection[vertice_index].coeffs[i];
            M_y[0] = M_y[0] + params[vertices_connection[vertice_index].faces[i]].M_y * vertices_connection[vertice_index].coeffs[i];
            E_x[0] = E_x[0] + params[vertices_connection[vertice_index].faces[i]].E_x * vertices_connection[vertice_index].coeffs[i];
        ...
        params[faces_connection[face].faces[j]].M_x = (factor * params[faces_connection[face].faces[j]].M_x + M_x[0] + M_x[1] + M_x[2]) / (factor + 3);
        params[faces_connection[face].faces[j]].M_y = (factor * params[faces_connection[face].faces[j]].M_y + M_y[0] + M_y[1] + M_y[2]) / (factor + 3);
        params[faces_connection[face].faces[j]].E_x = (factor * params[faces_connection[face].faces[j]].E_x + E_x[0] + E_x[1] + E_x[2]) / (factor + 3);
        params[faces_connection[face].faces[j]].E_y = (factor * params[faces_connection[face].faces[j]].E_y + E_y[0] + E_y[1] + E_y[2]) / (factor + 3);
        params[faces_connection[face].faces[j]].K_o_x = (factor * params[faces_connection[face].faces[j]].K_o_x + ko_x[0] + ko_x[1] + ko_x[2]) / (factor + 3);
        params[faces_connection[face].faces[j]].K_o_y = (factor * params[faces_connection[face].faces[j]].K_o_y + ko_y[0] + ko_y[1] + ko_y[2]) / (factor + 3);
        params[faces_connection[face].faces[j]].K_tau_xx = (factor * params[faces_connection[face].faces[j]].K_tau_xx + Ktau_xx[0] + Ktau_xx[1] + Ktau_xx[2]) / (factor + 3);
        params[faces_connection[face].faces[j]].K_tau_xy = (factor * params[faces_connection[face].faces[j]].K_tau_xy + Ktau_xy[0] + Ktau_xy[1] + Ktau_xy[2]) / (factor + 3);
        params[faces_connection[face].faces[j]].K_tau_yx = (factor * params[faces_connection[face].faces[j]].K_tau_yx + Ktau_yx[0] + Ktau_yx[1] + Ktau_yx[2]) / (factor + 3);
        params[faces_connection[face].faces[j]].K_tau_yy = (factor * params[faces_connection[face].faces[j]].K_tau_yy + Ktau_yy[0] + Ktau_yy[1] + Ktau_yy[2]) / (factor + 3);
...
void asignValues(int face,
    ...
    params[face].M_x = values[0]; params[face].M_y = values[1];
    params[face].E_x = values[2]; params[face].E_y = values[3];
    params[face].K_o_x = values[4]; params[face].K_o_y = values[5];
    params[face].K_tau_xx = values[6]; params[face].K_tau_xy = values[7];
    params[face].K_tau_yx = values[8]; params[face].K_tau_yy = values[9];
    params[face].J_xx = values[10]; params[face].J_xy = values[11];
    params[face].J_yx = values[12]; params[face].J_yy = values[13];

    for (i = 0; i < faces_connection[face].n; i++) {
        params[face].M_x = values[14 + 14 * i]; params[face].M_y = values[15 + 14 * i];
        params[face].E_x = values[16 + 14 * i]; params[face].E_y = values[17 + 14 * i];
        params[face].K_o_x = values[18 + 14 * i]; params[face].K_o_y = values[19 + 14 * i];
        params[face].K_tau_xx = values[20 + 14 * i]; params[face].K_tau_xy = values[21 + 14 * i];
        params[face].K_tau_yx = values[22 + 14 * i]; params[face].K_tau_yy = values[23 + 14 * i];
        params[face].J_xx = values[24 + 14 * i]; params[face].J_xy = values[25 + 14 * i];
        params[face].J_yx = values[26 + 14 * i]; params[face].J_yy = values[27 + 14 * i];
    }
...
void solveBoundaryLayer(int nf,
    ...
    double *norm_delta_list_aux; // normalized delta
    double *norm_A_list_aux;     // normalized A
    double *norm_B_list_aux;     // normalized B
    double *norm_Psi_list_aux;   // normalized Psi
    double *norm_Ctau1_list_aux; // normalized Ctau1
    double *norm_Ctau2_list_aux; // normalized Ctau2
    ...
    double *max_error_list;
    double avg_error;

    /* Initialize */
    int_max = 3000;

    max_error_list = (double *)malloc(int_max * sizeof(double));
    ...
    norm_delta_list_aux = (double *)malloc(nv * sizeof(double));
    norm_A_list_aux = (double *)malloc(nv * sizeof(double));
    norm_B_list_aux = (double *)malloc(nv * sizeof(double));
    norm_Psi_list_aux = (double *)malloc(nv * sizeof(double));
    norm_Ctau1_list_aux = (double *)malloc(nv * sizeof(double));
    norm_Ctau2_list_aux = (double *)malloc(nv * sizeof(double));

    double old_max_error;
    double ratio_max_error;
    ...
    double max_values[6] = {0.01, 0.001, 0.001, 0.001, 0.0001, 0.0001};
    double max_step;
    ...
    int face_max_error;
    double *avg_values = (double*)malloc(nf * sizeof(double));
    double diffusion_factor = 30.0;
    ...
    for (i = 0; i < int_max; i++) {
        ...
        // diffusion_factor = 100 / (1e-3 + max_error / max_obj_func);
        ...
        for (j = 0; j < nf; j++) {
            ...
            /* Average */
            // printf(" Antes: %.3e %.3e %.3e %.3e\n", params[j].M_x, params[j].M_y, params[j].E_x, params[j].E_y);
            avgValues(j, faces, faces_connection, vertices_connection, params, avg_values);
            // printf("Depois: %.3e %.3e %.3e %.3e\n", params[j].M_x, params[j].M_y, params[j].E_x, params[j].E_y);
            ...
            /* Asign avg values */
            asignValues(j, faces_connection, params, avg_values);
            ...
            /* Ref. obj. func. */
            calculateObjectiveFunction2(i, params[j], &obj_func[0], &max_momentum_x_aux, &max_momentum_y_aux, &max_kinetic_energy_aux, &max_lateral_curvature_aux, &max_shear_stress_x_aux, &max_shear_stress_y_aux, -1);
            ...
            max_step = step_ref;
            if (max_error < 1e-2) {
                for (k = 0; k < faces_connection[j].n + 1; k++) {
                    if (absValue(max_values[0] / (grad_obj_func[6 * k] + 1e-8)) < max_step) max_step = absValue(max_values[0] / (grad_obj_func[6 * k] + 1e-8));
                    if (absValue(max_values[1] / (grad_obj_func[6 * k + 1] + 1e-8)) < max_step) max_step = absValue(max_values[1] / (grad_obj_func[6 * k + 1] + 1e-8));
                    if (absValue(max_values[2] / (grad_obj_func[6 * k + 2] + 1e-8)) < max_step) max_step = absValue(max_values[2] / (grad_obj_func[6 * k + 2] + 1e-8));
                    if (absValue(max_values[3] / (grad_obj_func[6 * k + 3] + 1e-8)) < max_step) max_step = absValue(max_values[3] / (grad_obj_func[6 * k + 3] + 1e-8));
                    if (absValue(max_values[4] / (grad_obj_func[6 * k + 4] + 1e-8)) < max_step) max_step = absValue(max_values[4] / (grad_obj_func[6 * k + 4] + 1e-8));
                    if (absValue(max_values[5] / (grad_obj_func[6 * k + 5] + 1e-8)) < max_step) max_step = absValue(max_values[5] / (grad_obj_func[6 * k + 5] + 1e-8));
                }
            }
            step = max_step;
            ...
            norm_Ctau1_list[j] = norm_Ctau1_list[j] - step * 1.5 * grad_obj_func[4];
            norm_Ctau2_list[j] = norm_Ctau2_list[j] - step * 1.5 * grad_obj_func[5];

            if (norm_delta_list[j] < 0.0001) norm_delta_list[j] = 0.0001;
            if (norm_Ctau1_list[j] < 1e-15) norm_Ctau1_list[j] = 1e-15;
            if (norm_Ctau2_list[j] < 1e-15) norm_Ctau2_list[j] = 1e-15;
            ...
            for (k = 0; k < faces_connection[j].n; k++) {
                ...
                norm_Ctau1_list[faces_connection[j].faces[k]] = norm_Ctau1_list[faces_connection[j].faces[k]] - 1.5 * step * grad_obj_func[6 * (1 + k) + 4];
                norm_Ctau2_list[faces_connection[j].faces[k]] = norm_Ctau2_list[faces_connection[j].faces[k]] - 1.5 * step * grad_obj_func[6 * (1 + k) + 5];

                if (norm_delta_list[faces_connection[j].faces[k]] < 0.0001) norm_delta_list[faces_connection[j].faces[k]] = 0.0001;
                if (norm_Ctau1_list[faces_connection[j].faces[k]] < 1e-15) norm_Ctau1_list[faces_connection[j].faces[k]] = 1e-15;
                if (norm_Ctau2_list[faces_connection[j].faces[k]] < 1e-15) norm_Ctau2_list[faces_connection[j].faces[k]] = 1e-15;

            }
            
        }

        max_error_list[i - 1] = max_error;

        if (i > 1) {

            ratio_max_error = absValue(max_error - old_max_error) / max_error;

            if (division(max_error, old_max_error) <= 1) {
                step_ref = step_ref * 1.005;
            } else {
                step_ref = step_ref / 2.0;
            }

            // printf("%.4e, %.4e\n", ratio_max_error, step_ref);
        }
        ...
        if (i > 0) {
            ...
            for (j = 0; j < nv; j++) {
                ...
                norm_delta_list_aux[j] = 0.0;
                norm_A_list_aux[j] = 0.0;
                norm_B_list_aux[j] = 0.0;
                norm_Psi_list_aux[j] = 0.0;
                norm_Ctau1_list_aux[j] = 0.0;
                norm_Ctau2_list_aux[j] = 0.0;

                for (k = 0; k < vertices_connection[j].n; k++) {
                    norm_delta_list_aux[j] = norm_delta_list_aux[j] + norm_delta_list[vertices_connection[j].faces[k]] * vertices_connection[j].coeffs[k];
                    norm_A_list_aux[j] = norm_A_list_aux[j] + norm_A_list[vertices_connection[j].faces[k]] * vertices_connection[j].coeffs[k];
                    norm_B_list_aux[j] = norm_B_list_aux[j] + norm_B_list[vertices_connection[j].faces[k]] * vertices_connection[j].coeffs[k];
                    norm_Psi_list_aux[j] = norm_Psi_list_aux[j] + norm_Psi_list[vertices_connection[j].faces[k]] * vertices_connection[j].coeffs[k];
                    norm_Ctau1_list_aux[j] = norm_Ctau1_list_aux[j] + norm_Ctau1_list[vertices_connection[j].faces[k]] * vertices_connection[j].coeffs[k];
                    norm_Ctau2_list_aux[j] = norm_Ctau2_list_aux[j] + norm_Ctau2_list[vertices_connection[j].faces[k]] * vertices_connection[j].coeffs[k];
                }
            }

            for(j = 0; j < nf; j++) {
                norm_delta_list[j] = (diffusion_factor * norm_delta_list[j] + norm_delta_list_aux[faces[3 * j]] + norm_delta_list_aux[faces[3 * j + 1]] + norm_delta_list_aux[faces[3 * j + 2]]) / (diffusion_factor + 3);
                norm_A_list[j] = (diffusion_factor * norm_A_list[j] + norm_A_list_aux[faces[3 * j]] + norm_A_list_aux[faces[3 * j + 1]] + norm_A_list_aux[faces[3 * j + 2]]) / (diffusion_factor + 3);
                norm_B_list[j] = (diffusion_factor * norm_B_list[j] + norm_B_list_aux[faces[3 * j]] + norm_B_list_aux[faces[3 * j + 1]] + norm_B_list_aux[faces[3 * j + 2]]) / (diffusion_factor + 3);
                norm_Psi_list[j] = (diffusion_factor * norm_Psi_list[j] + norm_Psi_list_aux[faces[3 * j]] + norm_Psi_list_aux[faces[3 * j + 1]] + norm_Psi_list_aux[faces[3 * j + 2]]) / (diffusion_factor + 3);
                norm_Ctau1_list[j] = (diffusion_factor * norm_Ctau1_list[j] + norm_Ctau1_list_aux[faces[3 * j]] + norm_Ctau1_list_aux[faces[3 * j + 1]] + norm_Ctau1_list_aux[faces[3 * j + 2]]) / (diffusion_factor + 3);
                norm_Ctau2_list[j] = (diffusion_factor * norm_Ctau2_list[j] + norm_Ctau2_list_aux[faces[3 * j]] + norm_Ctau2_list_aux[faces[3 * j + 1]] + norm_Ctau2_list_aux[faces[3 * j + 2]]) / (diffusion_factor + 3);
            }
        ...
        if (max_error < 1e-8) {
            printf("BREAK: Minimum error limit! 1e-8");
            break;
        }
        if (obj_func[0] / max_obj_func > 100) {
            printf("BREAK: Maximum error limit! 1e3");
            break;
        }
        if (i >= 7) {
            if ((absValue(division(max_error_list[i - 1] - max_error, max_error)) < 1e-8) &&
                (absValue(division(max_error_list[i - 2] - max_error, max_error)) < 1e-8) &&
                (absValue(division(max_error_list[i - 3] - max_error, max_error)) < 1e-8) &&
                (absValue(division(max_error_list[i - 4] - max_error, max_error)) < 1e-8) &&
                (absValue(division(max_error_list[i - 5] - max_error, max_error)) < 1e-8) &&
                (absValue(division(max_error_list[i - 6] - max_error, max_error)) < 1e-8)) {
                printf("BREAK: The same error value on last 7 interactions!");
                break;
            }
        }
        // if (i >= 10) {
        //     if ((absValue(division(max_error_list[i - 1], max_error_list[i - 2])) > 1) &&
        //         (absValue(division(max_error_list[i - 2], max_error_list[i - 3])) > 1) &&
        //         (absValue(division(max_error_list[i - 3], max_error_list[i - 4])) > 1) &&
        //         (absValue(division(max_error_list[i - 5], max_error_list[i - 6])) > 1) &&
        //         (absValue(division(max_error_list[i - 6], max_error_list[i - 7])) > 1) &&
        //         (absValue(division(max_error_list[i - 7], max_error_list[i - 8])) > 1) &&
        //         (absValue(division(max_error_list[i - 8], max_error_list[i - 9])) > 1) &&
        //         (absValue(division(max_error_list[i - 9], max_error_list[i - 10])) > 1)) {
        //         printf("BREAK: Increasing error!");
        //         break;
        //     }
        // }

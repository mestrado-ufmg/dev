...
void calculateParametersGradients(int nf, int *faces, double *vertices, double *faceCenters, double *velx, double *vely, double *velz, double *transpiration, double *e3, struct VerticeConnection *vertices_connection, struct FaceGradients *facesGradients) {
    ...
    /* Parameters */
    double vel_aux_x, vel_aux_y, vel_aux_z;
    int face, i;
    ...
    for (face = 0; face < nf; face++) {
        ...
        for(i = 0; i < vertices_connection[faces[3 * face]].n; i++) {
            ...
            if ((e3[3 * face] * e3[3 * vertices_connection[faces[3 * face]].faces[i]] + e3[3 * face + 1] * e3[3 * vertices_connection[faces[3 * face]].faces[i] + 1] + e3[3 * face + 2] * e3[3 * vertices_connection[faces[3 * face]].faces[i] + 2]) < 0) {
                vel_aux_x = velx[face];
                vel_aux_y = vely[face];
                vel_aux_z = velz[face];
            } else {
                vel_aux_x = velx[vertices_connection[faces[3 * face]].faces[i]];
                vel_aux_y = vely[vertices_connection[faces[3 * face]].faces[i]];
                vel_aux_z = velz[vertices_connection[faces[3 * face]].faces[i]];
            }
        ...
        for(i = 0; i < vertices_connection[faces[3 * face + 1]].n; i++) {
            ...
            if ((e3[3 * face] * e3[3 * vertices_connection[faces[3 * face + 1]].faces[i]] + e3[3 * face + 1] * e3[3 * vertices_connection[faces[3 * face + 1]].faces[i] + 1] + e3[3 * face + 2] * e3[3 * vertices_connection[faces[3 * face + 1]].faces[i] + 2]) < 0) {
                vel_aux_x = velx[face];
                vel_aux_y = vely[face];
                vel_aux_z = velz[face];
            } else {
                vel_aux_x = velx[vertices_connection[faces[3 * face + 1]].faces[i]];
                vel_aux_y = vely[vertices_connection[faces[3 * face + 1]].faces[i]];
                vel_aux_z = velz[vertices_connection[faces[3 * face + 1]].faces[i]];
            }
        ...
        for(i = 0; i < vertices_connection[faces[3 * face + 2]].n; i++) {
            ...
            if ((e3[3 * face] * e3[3 * vertices_connection[faces[3 * face + 2]].faces[i]] + e3[3 * face + 1] * e3[3 * vertices_connection[faces[3 * face + 2]].faces[i] + 1] + e3[3 * face + 2] * e3[3 * vertices_connection[faces[3 * face + 2]].faces[i] + 2]) < 0) {
                vel_aux_x = velx[face];
                vel_aux_y = vely[face];
                vel_aux_z = velz[face];
            } else {
                vel_aux_x = velx[vertices_connection[faces[3 * face + 2]].faces[i]];
                vel_aux_y = vely[vertices_connection[faces[3 * face + 2]].faces[i]];
                vel_aux_z = velz[vertices_connection[faces[3 * face + 2]].faces[i]];
            }
...
void calculateParametersDivergents(int face, double *velNorm, int *faces, double *vertices, double *velx, double *vely, double *velz, double *transpiration, double *e3, struct VerticeConnection *vertices_connection, struct IntegralThickness params, struct FaceDivergents *faceDivergents) {
    ...
    for(i = 0; i < vertices_connection[faces[3 * face]].n; i++) {
        ...
        auxaa = (e3[3 * face] * e3[3 * vertices_connection[faces[3 * face]].faces[i]] + e3[3 * face + 1] * e3[3 * vertices_connection[faces[3 * face]].faces[i] + 1] + e3[3 * face + 2] * e3[3 * vertices_connection[faces[3 * face]].faces[i] + 2]);

        if (auxaa < 0) {

            // delta_1_ast / delta_2_ast
            delta_1_ast_1 = delta_1_ast_1 + params.delta_1_ast[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_1;
            delta_2_ast_1 = delta_2_ast_1 + params.delta_2_ast[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_1;

            // phi_11 / phi_12
            phi_11_1 = phi_11_1 + params.phi_11[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_2;
            phi_12_1 = phi_12_1 + params.phi_12[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_2;

            // phi_21 / phi_22
            phi_21_1 = phi_21_1 + params.phi_21[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_2;
            phi_22_1 = phi_22_1 + params.phi_22[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_2;

            // phi_1_ast / phi_2_ast
            phi_1_ast_1 = phi_1_ast_1 + params.phi_1_ast[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_3;
            phi_2_ast_1 = phi_2_ast_1 + params.phi_2_ast[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_3;

            // theta_1_o / theta_2_o
            theta_1_o_1 = theta_1_o_1 + params.theta_1_o[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_3;
            theta_2_o_1 = theta_2_o_1 + params.theta_2_o[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_3;

            // ktau_xx / ktau_xy
            ktau_xx_1 = ktau_xx_1 + params.Ktauxx[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_3;
            ktau_xy_1 = ktau_xy_1 + params.Ktauxy[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_3;

            ktau_yx_1 = ktau_yx_1 + params.Ktauyx[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_3;
            ktau_yy_1 = ktau_yy_1 + params.Ktauyy[face] * vertices_connection[faces[3 * face]].coeffs[i] * factor_3;
    ...
    for(i = 0; i < vertices_connection[faces[3 * face + 1]].n; i++) {
        ...
        auxaa = (e3[3 * face] * e3[3 * vertices_connection[faces[3 * face + 1]].faces[i]] + e3[3 * face + 1] * e3[3 * vertices_connection[faces[3 * face + 1]].faces[i] + 1] + e3[3 * face + 2] * e3[3 * vertices_connection[faces[3 * face + 1]].faces[i] + 2]);

        if (auxaa < 0) {

            // delta_1_ast / delta_2_ast
            delta_1_ast_2 = delta_1_ast_2 + params.delta_1_ast[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_1;
            delta_2_ast_2 = delta_2_ast_2 + params.delta_2_ast[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_1;

            // phi_11 / phi_12
            phi_11_2 = phi_11_2 + params.phi_11[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_2;
            phi_12_2 = phi_12_2 + params.phi_12[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_2;

            // phi_21 / phi_22
            phi_21_2 = phi_21_2 + params.phi_21[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_2;
            phi_22_2 = phi_22_2 + params.phi_22[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_2;

            // phi_1_ast / phi_2_ast
            phi_1_ast_2 = phi_1_ast_2 + params.phi_1_ast[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_3;
            phi_2_ast_2 = phi_2_ast_2 + params.phi_2_ast[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_3;

            // theta_1_o / theta_2_o
            theta_1_o_2 = theta_1_o_2 + params.theta_1_o[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_3;
            theta_2_o_2 = theta_2_o_2 + params.theta_2_o[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_3;

            // ktau_xx / ktau_xy
            ktau_xx_2 = ktau_xx_2 + params.Ktauxx[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_3;
            ktau_xy_2 = ktau_xy_2 + params.Ktauxy[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_3;

            ktau_yx_2 = ktau_yx_2 + params.Ktauyx[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_3;
            ktau_yy_2 = ktau_yy_2 + params.Ktauyy[face] * vertices_connection[faces[3 * face + 1]].coeffs[i] * factor_3;
    ...
    for(i = 0; i < vertices_connection[faces[3 * face + 2]].n; i++) {
        ...
        auxaa = (e3[3 * face] * e3[3 * vertices_connection[faces[3 * face + 2]].faces[i]] + e3[3 * face + 1] * e3[3 * vertices_connection[faces[3 * face + 2]].faces[i] + 1] + e3[3 * face + 2] * e3[3 * vertices_connection[faces[3 * face + 2]].faces[i] + 2]);

        if (auxaa < 0) {

            // delta_1_ast / delta_2_ast
            delta_1_ast_3 = delta_1_ast_3 + params.delta_1_ast[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_1;
            delta_2_ast_3 = delta_2_ast_3 + params.delta_2_ast[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_1;

            // phi_11 / phi_12
            phi_11_3 = phi_11_3 + params.phi_11[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_2;
            phi_12_3 = phi_12_3 + params.phi_12[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_2;

            // phi_21 / phi_22
            phi_21_3 = phi_21_3 + params.phi_21[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_2;
            phi_22_3 = phi_22_3 + params.phi_22[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_2;

            // phi_1_ast / phi_2_ast
            phi_1_ast_3 = phi_1_ast_3 + params.phi_1_ast[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_3;
            phi_2_ast_3 = phi_2_ast_3 + params.phi_2_ast[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_3;

            // theta_1_o / theta_2_o
            theta_1_o_3 = theta_1_o_3 + params.theta_1_o[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_3;
            theta_2_o_3 = theta_2_o_3 + params.theta_2_o[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_3;

            // ktau_xx / ktau_xy
            ktau_xx_3 = ktau_xx_3 + params.Ktauxx[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_3;
            ktau_xy_3 = ktau_xy_3 + params.Ktauxy[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_3;

            ktau_yx_3 = ktau_yx_3 + params.Ktauyx[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_3;
            ktau_yy_3 = ktau_yy_3 + params.Ktauyy[face] * vertices_connection[faces[3 * face + 2]].coeffs[i] * factor_3;
...
void calculateGradient(int face, struct FaceDivergents faceDivergents2, double *tau_w_x, double *tau_w_y, double density, struct FaceEquations *equations, double eps, double *velNorm, int *faces, double *vertices, double *velx, double *vely, double *velz, double *transpiration, double *e3, struct VerticeConnection *vertices_connection, struct FacesConnection *faces_connection, struct FaceGradients *facesGradients, double *gradient, struct IntegralThickness params, struct IntegralThickness params_delta, struct IntegralThickness params_A, struct IntegralThickness params_B, struct IntegralThickness params_Psi, struct IntegralThickness params_amp) {
    ...
    equations->obj = sqrt(equations->obj);
    ...
    obj_func_eps[0] = sqrt(obj_func_eps[0]);
    ...
    obj_func_eps[1] = sqrt(obj_func_eps[1]);
    ...
    obj_func_eps[2] = sqrt(obj_func_eps[2]);
    ...
    obj_func_eps[3] = sqrt(obj_func_eps[3]);
    ...
    obj_func_eps[4] = sqrt(obj_func_eps[4]);
...
void solveBoundaryLayer(int nf,
    ...
    for (i = 0; i < nf; i++) {
        ...
        A_list[i] = (1 / A_norm) * 0.0 * 0.332 * 5 * velNorm[i];
    ...
    for (i = 1; i <= max_interactions; i++) {
        ...
        /* Calculate parameters */
        for (j = 0; j < nf; j++) {
            calculateParameters(j, delta_norm * delta_list[j], A_norm * A_list[j], A_norm * B_list[j], Psi_norm * Psi_list[j], pow(10, -10 * amp_list[j]), velNorm[j], density, viscosity, mach[j], integralThickness);
            calculateParameters(j, delta_norm * (delta_list[j] + eps), A_norm * A_list[j], A_norm * B_list[j], Psi_norm * Psi_list[j], pow(10, -10 * amp_list[j]), velNorm[j], density, viscosity, mach[j], integralThickness_delta);
            calculateParameters(j, delta_norm * delta_list[j], A_norm * (A_list[j] + eps), A_norm * B_list[j], Psi_norm * Psi_list[j], pow(10, -10 * amp_list[j]), velNorm[j], density, viscosity, mach[j], integralThickness_A);
            calculateParameters(j, delta_norm * delta_list[j], A_norm * A_list[j], A_norm * (B_list[j] + eps), Psi_norm * Psi_list[j], pow(10, -10 * amp_list[j]), velNorm[j], density, viscosity, mach[j], integralThickness_B);
            calculateParameters(j, delta_norm * delta_list[j], A_norm * A_list[j], A_norm * B_list[j], Psi_norm * (Psi_list[j] + eps), pow(10, -10 * amp_list[j]), velNorm[j], density, viscosity, mach[j], integralThickness_Psi);
            calculateParameters(j, delta_norm * delta_list[j], A_norm * A_list[j], A_norm * B_list[j], Psi_norm * Psi_list[j], pow(10, -10 * (amp_list[j] + eps)), velNorm[j], density, viscosity, mach[j], integralThickness_amp);
        }
    ...
    for (i = 0; i < nf; i++) {
        ...
        Ctau1[i] = pow(10, -10 * amp_list[i]) / sqrt(1.4);
        Ctau2[i] = Ctau1[i];
    }
